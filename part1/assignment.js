...
function isVowel(letter) {
  if (letter === 'a' || letter === 'e' || letter === 'i' || letter === 'o' || letter === 'u' || letter === 'u') {
    return true;
  }else {
    return false;
  }
...
function stopAt(source, string) {
  return (source.substring(0,source.indexOf(string)).trim());
}
...
function capitalize(message) {
  ...
  var second = message.slice(1,message.length);

  first = first.toUpperCase();
  return first + second;*/

  return (message.substring(0,1).toUpperCase() + message.slice(1, message.length));
}
...
function calculateTaxRate(salary, status) {
  ...
  if (status === 'single') {
    if (salary < 9226) {
      return '10%';
    }else if (salary > 9225 && salary < 37451) {
      return '15%';
    }else if (salary > 37450 && salary < 74900) {
      return '25%';
    }
  }
  if (status === 'joint') {
    if (salary < 18451) {
      return '10%';
    }else if (salary > 18450 && salary < 74901) {
      return '15%';
    }
  }
  if (status !== 'single') {
